...
**关键参数说明：**
- **不要用 --headless**：无头模式会触发验证
- **不要用 --disable-gpu**：可能影响渲染
- **使用 --no-sandbox**：某些环境必须启用
...
1. **首次访问**：第一次访问CF保护网站可能需要等待几秒让验证完成
2. **Cookie复用**：验证通过后cookies会保存在user-data-dir中，后续访问无需重新验证
3. **窗口大小**：使用标准窗口大小(1280x800)避免被识别

### 结论

经过实测，最简单的方案就是使用**非无头模式的Chromium浏览器**，不需要复杂的参数配置即可通过Cloudflare验证。